...
final class ReportViewModel {
    ...
    // MARK: - Nahtstellen für `ReportViewModel+FileOps`

    /// **⚠️ Diese drei Methoden sind der Preis der Aufteilung — und die bessere
    /// Antwort auf ihn.**
    ///
    /// Swifts `private` gilt **dateiweit**: Eine Erweiterung in einer anderen
    /// Datei sieht es nicht. Gemessen an diesem Typ hieße eine vollständige
    /// Aufteilung, rund **dreißig** private Mitglieder zu `internal` zu
    /// erheben — `store`, `relevantFiles`, `invalidateRows`, `selectionAnchor`.
    /// Danach dürfte **jede Ansicht** sie anfassen. *Eine große Datei mit
    /// dichter Kapselung ist besser als sechs kleine ohne.*
    ///
    /// Deshalb wandert das Verhalten hinaus und der Zustand bleibt hier —
    /// erreichbar über **Absichten**, nicht über rohe Felder. Wer eine vierte
    /// Naht braucht, soll sie schreiben; wer statt dessen ein `private`
    /// entfernt, hat den Grund dieser Aufteilung aufgegeben.

    /// Der Bestand nach einem Ordner-Umzug — in **einem** Schritt gesetzt und gesichert.
    func applyRelocation(sources newSources: SourceList, pinned: [URL],
                         excluded: Set<String>, session: [URL]) {
        sources = newSources
        ...
        sessionCreatedFolders = session
    }

    /// Merkt sich, was ⌘Z zurücknehmen würde.
    func rememberUndo(_ pairs: [(from: URL, to: URL)], kind: TransferKind) {
        lastMove = pairs
        lastTransferKind = kind
    }

    /// Merkt den Ordner, aus dem gerade gezogen wird.
    func rememberDragOrigin(_ folder: URL) { dragOriginFolder = folder }

    /// Merkt einen selbst angelegten Ordner für diese Sitzung.
    func rememberCreatedFolder(_ url: URL) {
        sessionCreatedFolders.append(url)
    }
